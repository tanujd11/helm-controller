...
func TestTest_Reconcile(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectConditions: []metav1.Condition{
				*conditions.TrueCondition(meta.ReadyCondition, helmv2.TestSucceededReason,
		...
		},
		{
			...
			expectConditions: []metav1.Condition{
				*conditions.FalseCondition(meta.ReadyCondition, helmv2.TestFailedReason,
...
func TestTest_failure(t *testing.T) {
	...
	t.Run("records failure", func(t *testing.T) {
		...
		g.Expect(req.Object.Status.Conditions).To(conditions.MatchConditions([]metav1.Condition{
			*conditions.FalseCondition(helmv2.TestSuccessCondition, helmv2.TestFailedReason, expectMsg),
		}))
	...
	t.Run("records failure with logs", func(t *testing.T) {
		...
		g.Expect(conditions.IsFalse(req.Object, helmv2.TestSuccessCondition)).To(BeTrue())
		g.Expect(conditions.GetMessage(req.Object, helmv2.TestSuccessCondition)).ToNot(ContainSubstring(expectSubStr))
	...
	t.Run("increases remediation failure count", func(t *testing.T) {
		...
		obj := obj.DeepCopy()
		obj.Status.Current.SetTestHooks(map[string]*helmv2.HelmReleaseTestHook{})
	...
	t.Run("follows ignore failure instructions", func(t *testing.T) {
		...
		obj := obj.DeepCopy()
		obj.Spec.Test = &helmv2.Test{IgnoreFailures: true}
		obj.Status.Current.SetTestHooks(map[string]*helmv2.HelmReleaseTestHook{})
		req := &Request{Object: obj}
		r.failure(req, nil, err)
...
func TestTest_success(t *testing.T) {
	...
	t.Run("records success", func(t *testing.T) {
		...
		g.Expect(req.Object.Status.Conditions).To(conditions.MatchConditions([]metav1.Condition{
			*conditions.TrueCondition(helmv2.TestSuccessCondition, helmv2.TestSucceededReason, expectMsg),
		}))
	...
	t.Run("records success without hooks", func(t *testing.T) {
		...
		g.Expect(conditions.IsTrue(req.Object, helmv2.TestSuccessCondition)).To(BeTrue())
