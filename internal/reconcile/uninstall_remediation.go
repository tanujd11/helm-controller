...
func (r *UninstallRemediation) failure(req *Request, buffer *action.LogBuffer, err error) {
	...
	conditions.MarkFalse(req.Object, helmv2.RemediatedCondition, helmv2.UninstallFailedReason, msg)

	// Record warning event, this message contains more data than the
	// Condition summary.
	r.eventRecorder.AnnotatedEventf(req.Object, eventMeta(cur.ChartVersion), corev1.EventTypeWarning, helmv2.UninstallFailedReason, eventMessageWithLog(msg, buffer))
}
...
func (r *UninstallRemediation) success(req *Request) {
	...
	conditions.MarkTrue(req.Object, helmv2.RemediatedCondition, helmv2.UninstallSucceededReason, msg)

	// Record event.
	r.eventRecorder.AnnotatedEventf(req.Object, eventMeta(cur.ChartVersion), corev1.EventTypeNormal, helmv2.UninstallSucceededReason, msg)
}
