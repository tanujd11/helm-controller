...
func summarize(req *Request) {
	var sumConds = []string{helmv2.RemediatedCondition, helmv2.ReleasedCondition}
	if req.Object.GetTest().Enable && !req.Object.GetTest().IgnoreFailures {
		sumConds = []string{helmv2.RemediatedCondition, helmv2.TestSuccessCondition, helmv2.ReleasedCondition}
	}
	...
	if conds[0].Type == helmv2.RemediatedCondition {
...
func conditionallyDeleteRemediated(req *Request) {
	remediated := conditions.Get(req.Object, helmv2.RemediatedCondition)
	if remediated == nil {
	...
	released := conditions.Get(req.Object, helmv2.ReleasedCondition)
	...
	if !req.Object.GetTest().Enable || req.Object.GetTest().IgnoreFailures {
		...
		if released.Status == metav1.ConditionTrue && released.ObservedGeneration >= remediated.ObservedGeneration {
			conditions.Delete(req.Object, helmv2.RemediatedCondition)
		}
		return
	}

	testSuccess := conditions.Get(req.Object, helmv2.TestSuccessCondition)
