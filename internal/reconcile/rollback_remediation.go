...
func (r *RollbackRemediation) failure(req *Request, buffer *action.LogBuffer, err error) {
	...
	conditions.MarkFalse(req.Object, helmv2.RemediatedCondition, helmv2.RollbackFailedReason, msg)

	// Record warning event, this message contains more data than the
	// Condition summary.
	r.eventRecorder.AnnotatedEventf(req.Object, eventMeta(prev.ChartVersion), corev1.EventTypeWarning, helmv2.RollbackFailedReason, eventMessageWithLog(msg, buffer))
}
...
func (r *RollbackRemediation) success(req *Request) {
	...
	conditions.MarkTrue(req.Object, helmv2.RemediatedCondition, helmv2.RollbackSucceededReason, msg)

	// Record event.
	r.eventRecorder.AnnotatedEventf(req.Object, eventMeta(prev.ChartVersion), corev1.EventTypeNormal, helmv2.RollbackSucceededReason, msg)
}
