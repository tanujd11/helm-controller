...
func TestRollbackRemediation_Reconcile(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expectConditions: []metav1.Condition{
				*conditions.FalseCondition(meta.ReadyCondition, helmv2.RollbackSucceededReason, "Rolled back to"),
				*conditions.TrueCondition(helmv2.RemediatedCondition, helmv2.RollbackSucceededReason, "Rolled back to"),
			},
			expectCurrent: func(releases []*helmrelease.Release) *helmv2.HelmReleaseInfo {
		...
		},
		{
			...
			expectConditions: []metav1.Condition{
				*conditions.FalseCondition(meta.ReadyCondition, helmv2.RollbackFailedReason,
...
func TestRollbackRemediation_failure(t *testing.T) {
	...
	t.Run("records failure", func(t *testing.T) {
		...
		g.Expect(req.Object.Status.Conditions).To(conditions.MatchConditions([]metav1.Condition{
			*conditions.FalseCondition(helmv2.RemediatedCondition, helmv2.RollbackFailedReason, expectMsg),
		}))
	...
	t.Run("records failure with logs", func(t *testing.T) {
		...
		g.Expect(conditions.IsFalse(req.Object, helmv2.RemediatedCondition)).To(BeTrue())
		g.Expect(conditions.GetMessage(req.Object, helmv2.RemediatedCondition)).ToNot(ContainSubstring(expectSubStr))
...
func TestRollbackRemediation_success(t *testing.T) {
	...
	g.Expect(req.Object.Status.Conditions).To(conditions.MatchConditions([]metav1.Condition{
		*conditions.TrueCondition(helmv2.RemediatedCondition, helmv2.RollbackSucceededReason, expectMsg),
	}))
