...
func TestInstall_Reconcile(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectConditions: []metav1.Condition{
				*conditions.TrueCondition(meta.ReadyCondition, helmv2.InstallSucceededReason,
					"Installed release"),
		...
		},
		{
			...
			expectConditions: []metav1.Condition{
				*conditions.FalseCondition(meta.ReadyCondition, helmv2.InstallFailedReason,
					"failed post-install"),
...
func TestInstall_failure(t *testing.T) {
	...
	t.Run("records failure", func(t *testing.T) {
		...
		g.Expect(req.Object.Status.Conditions).To(conditions.MatchConditions([]metav1.Condition{
			*conditions.FalseCondition(helmv2.ReleasedCondition, helmv2.InstallFailedReason, expectMsg),
		}))
	...
	t.Run("records failure with logs", func(t *testing.T) {
		...
		g.Expect(conditions.IsFalse(req.Object, helmv2.ReleasedCondition)).To(BeTrue())
		g.Expect(conditions.GetMessage(req.Object, helmv2.ReleasedCondition)).ToNot(ContainSubstring(expectSubStr))
...
func TestInstall_success(t *testing.T) {
	...
	t.Run("records success", func(t *testing.T) {
		...
		g.Expect(req.Object.Status.Conditions).To(conditions.MatchConditions([]metav1.Condition{
			*conditions.TrueCondition(helmv2.ReleasedCondition, helmv2.InstallSucceededReason, expectMsg),
		}))
		g.Expect(recorder.GetEvents()).To(ConsistOf([]corev1.Event{
			{
	...
	t.Run("records success with TestSuccess=False", func(t *testing.T) {
		...
		obj := obj.DeepCopy()
		obj.Spec.Test = &helmv2.Test{Enable: true}

		req := &Request{Object: obj}
		r.success(req)

		g.Expect(conditions.IsTrue(req.Object, helmv2.ReleasedCondition)).To(BeTrue())

		cond := conditions.Get(req.Object, helmv2.TestSuccessCondition)
