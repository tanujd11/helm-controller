...
func TestUninstallRemediation_Reconcile(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			expectConditions: []metav1.Condition{
				*conditions.FalseCondition(helmv2.RemediatedCondition, helmv2.UninstallFailedReason, mockDeleteErr.Error()),
			},
			expectCurrent: func(releases []*helmrelease.Release) *helmv2.HelmReleaseInfo {
				return release.ObservedToInfo(release.ObserveRelease(releases[0]))
			},
...
func TestUninstallRemediation_failure(t *testing.T) {
	...
	t.Run("records failure", func(t *testing.T) {
		...
		g.Expect(req.Object.Status.Conditions).To(conditions.MatchConditions([]metav1.Condition{
			*conditions.FalseCondition(helmv2.RemediatedCondition, helmv2.UninstallFailedReason, expectMsg),
		}))
...
func TestUninstallRemediation_success(t *testing.T) {
	...
	g.Expect(req.Object.Status.Conditions).To(conditions.MatchConditions([]metav1.Condition{
		*conditions.TrueCondition(helmv2.RemediatedCondition, helmv2.UninstallSucceededReason, expectMsg),
	}))
	g.Expect(req.Object.Status.Failures).To(Equal(int64(0)))
	g.Expect(recorder.GetEvents()).To(ConsistOf([]corev1.Event{
		{
			Type:    corev1.EventTypeNormal,
			Reason:  helmv2.UninstallSucceededReason,
			Message: expectMsg,
			ObjectMeta: metav1.ObjectMeta{
				Annotations: map[string]string{
					"revision": cur.Chart.Metadata.Version,
