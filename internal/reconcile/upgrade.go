...
func (r *Upgrade) failure(req *Request, buffer *action.LogBuffer, err error) {
	...
	conditions.MarkFalse(req.Object, helmv2.ReleasedCondition, helmv2.UpgradeFailedReason, msg)

	// Record warning event, this message contains more data than the
	// Condition summary.
	r.eventRecorder.AnnotatedEventf(req.Object, eventMeta(req.Chart.Metadata.Version), corev1.EventTypeWarning, helmv2.UpgradeFailedReason, eventMessageWithLog(msg, buffer))
}
...
func (r *Upgrade) success(req *Request) {
	...
	conditions.MarkTrue(req.Object, helmv2.ReleasedCondition, helmv2.UpgradeSucceededReason, msg)
	if req.Object.GetTest().Enable && !cur.HasBeenTested() {
		conditions.MarkFalse(req.Object, helmv2.TestSuccessCondition, "Pending", fmtTestPending,
			cur.FullReleaseName(), cur.VersionedChartName())
	}

	// Record event.
	r.eventRecorder.AnnotatedEventf(req.Object, eventMeta(cur.ChartVersion), corev1.EventTypeNormal, helmv2.UpgradeSucceededReason, msg)
}
